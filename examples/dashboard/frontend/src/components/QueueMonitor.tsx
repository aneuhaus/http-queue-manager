          ...
          <tbody>
            {loading && !data ? (
              ...
              data?.requests.map((req: any) => (
                <tr key={req.id}>
                  <td><span className="badge">{req.method}</span></td>
                  <td><div style={{ maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={req.url}>{req.url}</div></td>
                  <td><span className={`badge ${req.status}`}>{req.status}</span></td>
                  <td>{req.attempts}</td>
                  <td>{new Date(req.createdAt).toLocaleTimeString()}</td>
                  <td className="text-error" style={{ maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={req.error}>{req.error}</td>
                </tr>
              ))
            )}
          </tbody>
